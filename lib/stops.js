...
function generateRecordXform(agencyId, agencyName, layerName) {
  return (rec, _, next) => {
    ...
    // "Upgrade" stop to station using a basic heuristic
    const lowercaseName = ( rec.stop_name || '' ).toLowerCase();
    if (lowercaseName.includes('station') || lowercaseName.includes('transit center')) {
      /* TODO: this should skip bays of stations while also allowing water bays to be stations
         however, this is quite fragile
        
         If a stop is a bay of a larger station, it will *generally* have a parent stop
      */
      if(!lowercaseName.includes('(bay') && !lowercaseName.includes('- bay') && !rec.parent_station) {
        layerId = 'stations';
      }
    }
    ...
    const doc = utils.makePeliasRecord(layerId, id, name, rec.stop_lat, rec.stop_lon);
